...
日本語
----------
Replitを開く: replit.com にアクセスしてログインします（無料アカウントでOKです）。
インポート: 左サイドバーの "Import code or design" をクリックし、"GitHub" を選択します。
URL貼り付け: https://github.com/MainCharacter55/Account-Management-System を貼り付け、"Import from GitHub" をクリックします。
APIキーの取得: exchangerate-api.com で無料アカウントを作成し、"Your API Key" をコピーします。

シークレットの設定:
Replit内のツールから "Secrets (🔒)" を開きます。
"New configuration" をクリックします。
Keyに EXCHANGE_RATE_API_KEY と入力します。
Valueにコピーした APIキー を貼り付け、"Add configuration" をクリックします。

実行:
"Shell" タブを開きます。
左上の実行ボタンをタップして1分待ちます。

以下のコマンドを入力して実行します：
python 口座管理簿/main.py

開始: 画面の指示に従ってアカウントを作成し、管理を開始してください！
